...
function installControllers() {
    ...
    for (let gkey in qview.controllers) {
        ...
        elem.onclick = () => {
            if (view.currentKey != "") {
                dom.getElementById(view.currentKey).removeAttribute("style");
            }
        };
...
function installPropSelectors() {
    document.getElementById("menu").insertAdjacentElement("afterend", `<br><div id="properties">
...
function installMousePos() {
    document.getElementById("properties").insertAdjacentElement("beforeend", `&nbsp;<span id="mousepos"></span>`);
    let old = view.drawing.onmousemove;
    let mousepos = document.getElementById("mousepos");
    view.drawing.onmousemove = event => {
